...
type NoteItem  = { id: string; hr_code: string; note_text: string; status: string; created_at: string };
type SessionReport = {
  id: string;
...
export default function AdminReportsView({ sessions: initialSessions }: { sessions: SessionReport[] }) {
  ...
  async function updateNoteStatus(id: string, status: string, sessionId: string) {
    setSaving(id);
    await supabase.from("session_notes").update({ status, updated_at: new Date().toISOString() }).eq("id", id);
    setSessions((prev) => prev.map((s) =>
      s.id === sessionId ? { ...s, notes: s.notes.map((n) => n.id === id ? { ...n, status } : n) } : s
    ));
    setSaving(null);
  }
  ...
  const visible = sessions.filter((s) => {
    ...
    return s.match_name.toLowerCase().includes(q) ||
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Match session reports — videos, collector notes and acknowledgements.</p>
        </div>
        <Link href="/upload" className="rounded-lg bg-slate-900 text-white px-4 py-2 text-sm font-medium hover:bg-slate-700">
          Upload New Report →
        </Link>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 flex flex-wrap gap-4 items-end">
        ...
        {(search || noteFilter) && (
          <button type="button" onClick={() => { setSearch(""); setNoteFilter(""); }}
      ...
      {visible.length === 0 ? (
        <p className="text-slate-500">No reports yet. <Link href="/upload" className="text-blue-600 underline">Upload one now.</Link></p>
      ) : (
        <div className="space-y-2">
          {visible.map((s) => {
            const isExp = expandedId === s.id;
            const visibleNotes = s.notes.filter((n) => noteFilter ? n.status === noteFilter : true);
            return (
              <div key={s.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <button type="button" onClick={() => setExpandedId(isExp ? null : s.id)}
                  ...
                  <div className="border-t border-slate-100 px-5 pb-5 pt-4 space-y-5">
                    ...
                    <div>
                      ...
                      {visibleNotes.length === 0 ? (
                        <p className="text-slate-500 text-sm">{noteFilter ? "No notes matching this status." : "No notes yet."}</p>
                      ) : (
                              ...
                              {visibleNotes.map((n) => (
                                <tr key={n.id} className="border-t border-slate-100 align-top">
                                  ...
                                  <td className="px-3 py-2.5 whitespace-nowrap">
                                    <select value={n.status} disabled={saving === n.id}
                                      ...
                                      {NOTE_STATUSES.map((st) => <option key={st} value={st}>{st}</option>)}
                                    </select>
                                  </td>
